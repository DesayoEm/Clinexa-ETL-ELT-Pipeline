...
def transform_adverse_events(self, study_key: str, study_data: pd.Series) -> Tuple:
    ...
    events_index = NESTED_FIELDS["adverse_events"]["index_field"]
    ...
    if isinstance(event_data, dict) and event_data:
        ...
        if isinstance(event_groups, (list, np.ndarray)) and len(event_groups) > 0:
            for event_group in event_groups:
                ...
                event_group_key = self.generate_key(study_key, adverse_event_key, group_id)
        ...
        if isinstance(serious_events_list, (list, np.ndarray)) and len(serious_events) > 0:
            for serious_event in serious_events_list:
                ...
                serious_event_key = self.generate_key(study_key, adverse_event_key, term)
                ...
                if isinstance(serious_event_stats_list, (list, np.ndarray)) and len(serious_event_stats_list) > 0:
                    for serious_event_stat in serious_event_stats_list:
                        group_id = serious_event_stat.get('groupId')
                        event_stat_key = self.generate_key(study_key, adverse_event_key, serious_event_key, group_id)
        ...
        if isinstance(other_events_list, (list, np.ndarray)) and len(other_events_list) > 0:
            for other_event in other_events_list:
                ...
                if isinstance(other_event_stats_list, (list, np.ndarray)) and len(other_event_stats_list) > 0:
                    for other_event_stat in other_event_stats_list:
                        
                        group_id = other_event_stat.get('groupId')
                        event_stat_key = self.generate_key(study_key, adverse_event_key, other_event_key, group_id)
    ...
    return adverse_event, event_groups, serious_events, serious_event_stats, other_events, other_event_stats
