...
def transform_outcome_measures(study_key: str, study_data: pd.Series) -> Tuple:
    ...
    if isinstance(outcomes_measures_list, (list, np.ndarray)) and len(outcomes_measures_list) > 0:
        for outcome_measure in outcomes_measures_list:
            ...
            outcome_measure_key = generate_key(study_key, outcome_measure_title, outcome_measure_type)
            ...
            outcome_measures.append({
                ...
                'population_description': outcome_measure.get('populationDescription'),
                'reporting_status': outcome_measure.get('reportingStatus'),
                'anticipated_posting_date': outcome_measure.get('anticipatedPostingDate'),
            ...
            if isinstance(groups, (list, np.ndarray)) and len(groups) > 0:
                for group in groups:
                    ...
                    outcome_group_key = generate_key(study_key, outcome_measure_key, group_id)
                ...
                for denom in denoms_list:
                    ...
                    if isinstance(denom_counts, (list, np.ndarray)) and len(denom_counts) > 0:
                        for denom_count in denom_counts:
                            ...
                            denom_count_group_key = generate_key(study_key, outcome_measure_key, denom_count_group_id)
                            denom_count_key = generate_key(study_key, outcome_measure_key, denom_unit_key,
            ...
            if isinstance(classes, (list, np.ndarray)) and len(classes) > 0:
                categories = classes[0].get('categories') #class is not a true container and only wraps categories contrary to what the docs say
                    ...
                    for cat_idx, category in enumerate(categories):
                        ...
                        if isinstance(measurements, (list, np.ndarray)) and len(measurements) > 0:
                            for measurement in measurements:
                                ...
                                meas_group_key = generate_key(study_key, outcome_measure_key, meas_group_id)
            ...
            if isinstance(analyses, (list, np.ndarray)) and len(analyses) > 0:
                for analysis in analyses:
                    ...
                    analysis_key = generate_key(study_key, outcome_measure_key, param_type, param_value)
                    ...
                    if isinstance(analysis_comparison_groups, (list, np.ndarray))  and len(analysis_comparison_groups) > 0:
                        for group in analysis_comparison_groups:
                            outcome_measure_comparison_groups.append({
                                'analysis_key': analysis_key,
                                'outcome_measure_key': group.get('outcomeMeasureKey'),
                                'study_key': study_key,
    ...
    return ( outcome_measures, outcome_measure_groups, outcome_measure_denom_units, outcome_measure_denom_counts,outcome_measure_measurements,
             outcome_measure_analyses, outcome_measure_comparison_groups
